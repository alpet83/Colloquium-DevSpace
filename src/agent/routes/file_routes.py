# /agent/routes/file_routes.py, updated 2025-07-14 19:12 EEST
...
async def upload_file(request: Request, file: UploadFile = File(...), chat_id: int = Form(...), file_name: str = Form(...)):
    logging.debug(f"#DEBUG: Запрос POST /chat/upload_file, IP={request.client.host}, Cookies={request.cookies}")
    try:
        ...
        result = globals.file_manager.upload_file(chat_id, user_id, content, file_name)
        logging.debug(f"#DEBUG: Файл загружен для chat_id={chat_id}, user_id={user_id}, file_name={file_name}")
        return result
...
async def update_file(request: Request, file: UploadFile = File(...), file_id: int = Form(...), file_name: str = Form(...)):
    logging.debug(f"#DEBUG: Запрос POST /chat/update_file, IP={request.client.host}, Cookies={request.cookies}")
    try:
        session_id = request.cookies.get("session_id")
        if not session_id:
            logging.info(f"#INFO: Отсутствует session_id для IP {request.client.host}")
        ...
        user_id = user_id[0]
        content = await file.read()
        result = globals.file_manager.update_file(file_id, user_id, content, file_name)
        logging.debug(f"#DEBUG: Файл file_id={file_id} обновлён для user_id={user_id}, file_name={file_name}")
        return result
    except HTTPException as e:
...
async def list_files(request: Request):
    logging.debug(f"#DEBUG: Запрос GET /chat/list_files, IP={request.client.host}, Cookies={request.cookies}")
    try:
        session_id = request.cookies.get("session_id")
        if not session_id:
            logging.info(f"#INFO: Отсутствует session_id для IP {request.client.host}")
        ...
        if not user_id:
            ...
            return {"error": "Invalid session"}
        user_id = user_id[0]
        files = globals.file_manager.list_files(user_id)
        logging.debug(f"#DEBUG: Возвращено {len(files)} файлов для user_id={user_id}")
        return files
    except HTTPException as e:
...
@router.get("/chat/get_sandwiches_index")
async def get_sandwiches_index(request: Request):
    logging.debug(f"#DEBUG: Запрос GET /chat/get_sandwiches_index, IP={request.client.host}, Cookies={request.cookies}")
    try:
        session_id = request.cookies.get("session_id")
        if not session_id:
            logging.info(f"#INFO: Отсутствует session_id для IP {request.client.host}")
        ...
        if not user_id:
            ...
            return {"error": "Invalid session"}
        user_id = user_id[0]
        result = globals.file_manager.get_sandwiches_index()
        logging.debug(f"#DEBUG: Возвращён индекс сэндвичей для user_id={user_id}")
        return result
    except HTTPException as e:
        logging.error(f"#ERROR: HTTP ошибка в GET /chat/get_sandwiches_index: {str(e)}")
        raise
    except Exception as e:
        logging.error(f"#ERROR: Ошибка сервера в GET /chat/get_sandwiches_index: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Server error: {str(e)}")
