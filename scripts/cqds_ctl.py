...
class CqdsCtl:
    ...
    def clear_log_via_container(self, log_path: str) -> tuple[int | None, int | None, bool, str | None]:
        """Stat + truncate a Docker json-file log by running a short-lived privileged container.
        ...
        Returns (bytes_before, bytes_after, ok, error_message).
        """
        ...
        try:
            ...
            if r.returncode != 0:
                return bytes_before, None, False, (r.stderr or r.stdout).strip()[:200]

            bytes_after, err_after = self._stat_log_size_via_container(log_dir, log_file)
            if err_after:
                return bytes_before, None, False, err_after
            return bytes_before, bytes_after, bytes_after == 0, None
        except Exception as exc:
            return bytes_before, None, False, str(exc)[:120]
