...
3. **6 MCP Tool Handlers** in cqds_mcp_full.py:
...
### 2. cqds_mcp_full.py (p:\opt\docker\cqds\mcp-tools\cqds_mcp_full.py)
...
Copilot Chat Request
    ↓
MCP Tool Handler (cqds_mcp_full.py)
    ↓
...
All test files live in the mcp-sandbox service and can be executed via:
```bash
docker compose exec mcp-sandbox python3 /app/projects/test_comprehensive.py
```
